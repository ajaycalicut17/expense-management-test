...
test('total expenses per category', async ({ page }) => {
  ...
  await expect(page.locator('#expenseByCategory')).toHaveScreenshot('start-dashboard-expense-by-category-march-2024.png', { threshold: 0.02 });
  ...
  await expect(page.locator('#expenseByCategory')).toHaveScreenshot('end-dashboard-expense-by-category-march-2024.png', { threshold: 0.02 });
  await expect(page.getByText('March 2024')).toBeVisible();
});
